...
contract Vault is Pausable {
    ...
    event ContributorsUpdated(
        address newContributor, uint256 indexed oldNumContributors, uint256 indexed newNumContributors
    );
    ...
    function deposit(uint72 amount) external payable whenNotPaused {
        ...
        if (individualContributions[msg.sender] == 0) {
            emit ContributorsUpdated(msg.sender, numContributors, numContributors++);
        }
